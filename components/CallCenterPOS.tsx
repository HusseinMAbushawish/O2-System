    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-slate-900 rounded-2xl p-5 w-full max-w-sm border border-white/10" onClick={e => e.stopPropagation()} dir="rtl">
        ...
        <div className="bg-slate-800/80 rounded-xl p-3 mb-3 flex items-center justify-between">
          ...
          <div className="text-left"><p className="text-[10px] text-slate-500">الإجمالي</p><p className="text-lg font-black text-red-500">{total > 0 ? `${total.toFixed(1)} شيكل` : '---'}</p></div>
        </div>
        <button onClick={() => { if (weight > 0) onConfirm(weight, total); }} disabled={weight <= 0}
          className="w-full py-3 bg-red-600 text-white rounded-xl font-black text-sm hover:bg-red-700 disabled:opacity-30 disabled:cursor-not-allowed">إضافة للفاتورة</button>
      </motion.div>
    </motion.div>
  );
};
...
export const CallCenterPOS: React.FC = () => {
  ...
  const [rightTab, setRightTab] = useState<'context' | 'complaints' | 'drivers'>('context');
  ...
  const handleSubmitOrder = () => {
    ...
    submitOrder(OrderStatus.CONFIRMED, paymentMethod, discountValue, {
    ...
    setFoundCustomer(null); setPhoneInput(''); setOrderNote(''); setDiscountAmount(0);
  };
  ...
  return (
    <div className="h-full flex gap-0 overflow-hidden bg-slate-950" dir="rtl">
      ...
      {/* ═══════════════════════════════════════════ */}
      {/* ═══  LEFT COLUMN: Nav + Live Stats     ═══ */}
      {/* ═══════════════════════════════════════════ */}
      <div className="w-[60px] flex flex-col bg-slate-900 border-l border-white/5 items-center py-3 gap-3 flex-shrink-0">
        {/* Live Stats - compact vertical */}
        <div className="flex flex-col items-center gap-1 w-full px-1.5">
          <div className="w-full bg-slate-800 rounded-lg p-2 text-center" title="طلبات اليوم">
            <ShoppingBag size={14} className="text-red-500 mx-auto mb-0.5" />
          ...
          <div className="w-full bg-slate-800 rounded-lg p-2 text-center" title="متوسط التوصيل">
            <Timer size={14} className="text-amber-500 mx-auto mb-0.5" />
          ...
          <div className="w-full bg-slate-800 rounded-lg p-2 text-center" title="شكاوي مفتوحة">
            <AlertTriangle size={14} className={`mx-auto mb-0.5 ${openComplaints > 0 ? 'text-red-500' : 'text-emerald-500'}`} />
          ...
          <div className="w-full bg-slate-800 rounded-lg p-2 text-center" title="سائقين نشطين">
            <Bike size={14} className="text-sky-500 mx-auto mb-0.5" />
      ...
      <div className="flex-1 flex flex-col min-w-0 border-l border-white/5">
        ...
        {/* Top Bar: Phone Search + Cart Summary */}
        <div className="flex-shrink-0 flex items-center gap-3 px-4 py-2.5 bg-slate-900/80 border-b border-white/5">
          {/* Phone Input */}
          ...
          <div className="w-[280px] flex flex-col bg-slate-900/40 border-r border-white/5 flex-shrink-0">
            ...
            <div className="flex-shrink-0 border-t border-white/5">
              ...
              <div className="px-2.5 py-2 border-t border-white/10 bg-slate-900/60 space-y-1">
                ...
                {discountValue > 0 && <div className="flex justify-between text-[10px]"><span className="text-emerald-500">الخصم</span><span className="text-emerald-400 font-black">-{discountValue.toFixed(1)}</span></div>}
                ...
                <div className="flex justify-between items-baseline"><span className="text-white font-black text-xs">الإجمالي</span><span className="text-red-500 font-black text-base">{grandTotal.toFixed(1)}</span></div>
              </div>
      ...
      {/* ═══════════════════════════════════════════ */}
      <div className="w-[300px] flex flex-col bg-slate-900 flex-shrink-0 overflow-hidden">
        {/* Tabs */}
        <div className="flex-shrink-0 flex border-b border-white/5">
          {[
            ...
            { id: 'drivers' as const, label: 'السائقين', icon: BarChart3 },
          ].map(tab => (
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
              ...
              {foundCustomer ? (
                <>
                  ...
                  <div className="bg-slate-800/40 rounded-xl p-3 border border-white/[0.03]">
                    ...
                    {/* Favorite + Last Order - Always visible */}
                    <div className="grid grid-cols-2 gap-1.5 mb-2">
                      <div className="bg-slate-800/60 rounded-lg p-2">
                        <p className="text-[8px] text-slate-600 mb-0.5">المفضل</p>
                        <p className="text-[10px] font-black text-amber-400 truncate">{foundCustomer.favoriteItem || '---'}</p>
                      </div>
                      <div className="bg-slate-800/60 rounded-lg p-2">
                        <p className="text-[8px] text-slate-600 mb-0.5">آخر طلب</p>
                    ...
                    <button onClick={() => setShowMoreProfile(!showMoreProfile)}
                      ...
                      {showMoreProfile ? <><EyeOff size={10} /> إخفاء التفاصيل</> : <><Eye size={10} /> عرض المزيد</>}
                    </button>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <div className="pt-2 space-y-1.5">
                            ...
                            <div className="grid grid-cols-2 gap-1.5">
                              <div className="bg-slate-800/60 rounded-lg p-2">
                                <p className="text-[8px] text-slate-600">الطلبات</p>
                                <p className="text-xs font-black text-white">{foundCustomer.totalOrders}</p>
                              </div>
                              <div className="bg-slate-800/60 rounded-lg p-2">
                                <p className="text-[8px] text-slate-600">الإنفاق</p>
                                <p className="text-xs font-black text-white">{foundCustomer.totalSpending}&#8362;</p>
                              </div>
                              <div className="bg-slate-800/60 rounded-lg p-2">
          ...
          {rightTab === 'drivers' && (
            <div className="p-3">
              <h4 className="text-[10px] font-black text-slate-500 mb-2 flex items-center gap-1"><BarChart3 size={11} /> أداء السائقين</h4>
              <DriverTable drivers={deliveryEmployees} />
            </div>
          )}
        </div>
      </div>
